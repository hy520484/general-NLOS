function target = NLOS_adjoint(Sig_flag, xmin_i,ymin_i,zmin_i,xmin_d,ymin_d,zmin_d, xg,yg,zg, C_i, C_d, Arr_bin_i,Arr_bin_d, Des_value_i, Des_value_d, Sig)
% This function realizes the adjoint operator of the physical model, which 
% corresponds to the back-projection operator. 
lxs = length(xg); lys = length(yg); lzs = length(zg);
num_measure = size(Sig,1);

target = zeros(lxs,lys,lzs,3);
...
parfor s = 1:num_measure
       ...
       target = target + repmat(temp_sig(temp_Arr),1,1,1,3) .* temp_Des_d .* repmat(temp_Des_i,1,1,1,3);
