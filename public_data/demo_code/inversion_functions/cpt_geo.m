function [Arr_bin_i,Arr_bin_d, Des_value_i, Des_value_d, min_bin, max_bin, xmin_i, ymin_i, zmin_i, xmin_d, ymin_d, zmin_d] = cpt_geo(c,delta_t, xbc,ybc,zbc, bx,by,bz, xec,yec,zec, C_i, C_d)
% This function generates the physical operator.
...
Des_value_d = repmat( 1 ./ (n_vec_d.^3) ,1,1,1,3);
Des_value_d(:,:,:,1) = KK_d .* Des_value_d(:,:,:,1);
Des_value_d(:,:,:,2) = JJ_d .* Des_value_d(:,:,:,2);
Des_value_d(:,:,:,3) = LL_d .* Des_value_d(:,:,:,3);
Des_value_d = - Des_value_d;
